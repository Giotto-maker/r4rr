...
# ! Baseline with pretraining option for Kandinsky using joint shape/colour encoder
import sys
import os
import torch
import signal
...
from protonet_kand_modules.utility_modules.pretraining_joint import pre_train
...
def train(model: MnistDPL,
    ...
    for epoch in range(args.proto_epochs + 1):  # first epoch is for determining the baseline accuracy
        ...
        tloss, cacc, yacc, f1 = evaluate_metrics(
            model, val_loader, args, concept_extractor=concept_extractor, transform=transform
        )
