...
BASELINE_DIR_JOINT = '../outputs++/bddoia/baseline/disj/dpl'
BASELINE_DIR_DISJ = '../outputs++/bddoia/baseline/joint/dpl'
...
y_names = {
    ...
    'Mean of collapses': 'Collapse Mean'
}

baseline_color_disj = 'royalblue'
baseline_color_joint = 'darkgoldenrod'
...
baseline_constant_file = os.path.join(BASELINE_DIR_JOINT, 'baseline-plain-1.0', 'aggregated_metrics.txt')
...
for v in versions:
    ...
    base_folder = os.path.join(BASELINE_DIR_JOINT, f"baseline-c-{v}")
    ...
    # Proto folder name
    proto_folder = os.path.join(BASELINE_DIR_DISJ, f"baseline-c-{v}")
...
# Plotting
output_dir = './.pdfs/bddoia-baselines'
...
for metric in metrics:
    ...
    # Baseline line
    plt.plot(x, y_base, label='DDPL+C', linestyle='dashed')
    plt.fill_between(x,
                     np.array(y_base) - np.array(y_base_std),
                     np.array(y_base) + np.array(y_base_std), alpha=0.2, color=baseline_color_disj)
    # Proto line
    plt.plot(x, y_proto, label='DPL+C', linestyle='dashdot')
    plt.fill_between(x,
                     np.array(y_proto) - np.array(y_proto_std),
                     np.array(y_proto) + np.array(y_proto_std), alpha=0.2, color=baseline_color_joint)
