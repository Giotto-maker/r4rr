...
BASE_DIR_AUG = os.path.join(os.getcwd(), '../outputs++/mnadd-even-odd', 'baseline')
BASE_DIR_PROTO = os.path.join(os.getcwd(), '../NEW-outputs/mnadd-even-odd', 'my_models')

model_names = ['sl', 'ltn', 'dpl']

# Use distinct colors for pretraining and Proto (PNet)
color_map_aug = {
    'sl': 'royalblue',
    'ltn': 'forestgreen',
    'dpl': 'darkorange'
...
legend_labels = {
    'sl_pretraining': "SL+Pre*+C(S)*",
    'ltn_pretraining': "LTN+Pre*+C(S)*",
    'dpl_pretraining': "DPL+Pre*+C(S)*",
    'sl_proto': "SL+PNet",
    'ltn_proto': "LTN+PNet",
    'dpl_proto': "DPL+PNet"
}
...
# Regexes
version_regex_aug = re.compile(r'pretrained-(\d+\.\d+)\+c$')
version_regex_proto = re.compile(r'episodic-proto-net-pipeline-(\d+\.\d+)-HIDE-\[\]-NA:True$')
...
def load_data(base_dir, version_regex, is_proto=False):
    ...
    for model in model_names:
        if model == 'ltn' and not is_proto:  
            # * baseline LTN works better in the standard pretraining setting  
            version_regex = re.compile(r'Apretrained-(\d+\.\d+)$')
        ...
        key_prefix = f"{model}_{'proto' if is_proto else 'pretraining'}"
        data[key_prefix] = {
            'versions': versions,
...
def plot_metric(metric, filename, break_at=None, baseline_scores=None, model_type=None):
    ...
    assert model_type == 'proto' or model_type == 'pretraining' 
    
    y_lower = min([v[metric][0][0] for v in data_all.values() if len(v[metric]) > 0])
    y_lower = max(y_lower, 0.0)  # Ensure y_lower is not below 0
    ...
    if metric == 'Concept' and model_type == 'pretraining':
        ax.set_ylim(top=1.0)
    ...
    if baseline_scores and model_type=='pretraining':
    ...
    if (metric == 'Collapse' and model_type == 'proto'):# or ((metric == 'ConceptAcc' or metric == 'Concept') and model_type == 'pretraining'):
...
# === Generate Plots ===
plot_metric('Concept', 
            '.pdfs/mnist-plots++/concept_f1_combined_proto.pdf', 
            break_at=None, 
            model_type='proto', 
            baseline_scores=baseline_scores)
plot_metric('Label', 
            '.pdfs/mnist-plots++/label_f1_combined_proto.pdf', 
            break_at=None, model_type='proto', 
            baseline_scores=baseline_scores)
plot_metric('Concept', 
            '.pdfs/mnist-plots++/concept_f1_combined_aug.pdf', 
            break_at=None, 
            model_type='pretraining', 
            baseline_scores=baseline_scores)
plot_metric('Label', 
            '.pdfs/mnist-plots++/label_f1_combined_aug.pdf', 
            break_at=None, 
            model_type='pretraining', 
            baseline_scores=baseline_scores)
plot_metric('ConceptAcc', 
            '.pdfs/mnist-plots++/concept_acc_combined_proto.pdf', 
            break_at=None, 
            model_type='proto', 
            baseline_scores=baseline_scores)
plot_metric('LabelAcc', 
            '.pdfs/mnist-plots++/label_acc_combined_proto.pdf', 
            break_at=None, 
            model_type='proto', 
            baseline_scores=baseline_scores)
plot_metric('ConceptAcc', 
            '.pdfs/mnist-plots++/concept_acc_combined_aug.pdf', 
            break_at=None, 
            model_type='pretraining', 
            baseline_scores=baseline_scores)
plot_metric('LabelAcc', 
            '.pdfs/mnist-plots++/label_acc_combined_aug.pdf', 
            break_at=None, 
            model_type='pretraining', 
            baseline_scores=baseline_scores)
plot_metric('Collapse', 
            f'.pdfs/mnist-plots++/collapse_combined_proto_{collapse_flag}.pdf', 
            break_at=None, 
            model_type='proto', 
            baseline_scores=baseline_scores)
plot_metric('Collapse', 
            f'.pdfs/mnist-plots++/collapse_combined_aug_{collapse_flag}.pdf', 
            break_at=None, 
            model_type='pretraining', 
            baseline_scores=baseline_scores)