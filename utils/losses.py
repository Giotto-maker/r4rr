...
def ADDMNIST_Classification(out_dict: dict, args):
    ...
    if args.model in [
        ...
        raise RuntimeError(f"Model {args.model} not recognized: loss not computed.")
...
def KAND_Classification(out_dict: dict, args):
    ...
    if args.model in [
        "kanddpl",
        "prokanddpl",   # ! new
        "kanddplsinglejoint", # ! new
        "kanddplsingledisj", # ! new 
        "kandcbm",
        "kandnn",
        "kandclip",
        "minikanddpl",
    ...
    elif args.model in ['kandsl',
                        'prokandsl',
                        'prokandsloneembedding',
                        'kandsloneembedding',
                        'kandslsinglejoint',
                        'kandslsingledisj']:
        loss = F.cross_entropy(out, final_labels, reduction="mean")
    else:
        raise ValueError(f"Model {args.model} not recognized: loss not computed.")
        loss = torch.tensor(1e-5)
...
def SDDOIA_Concept_Match(out_dict: dict, args):
    ...
    for i, rep in enumerate(probs_list):
        ...
        if mask.sum() > 0:  # Proceed only if there are valid entries
            ...
            print(f"Skipping concept supervision for index {i} due to no valid concepts.")
...
def SDDOIA_Cumulative(out_dict: dict, args):
    ...
    apply_kl = (
        args.dataset == 'boia_original_embedded'
        and args.expressive_model
        and args.prototypes
        and args.w_kl > 0
    )
    ...
    if apply_kl:
        ...
        kl_total = 0.0
        eps = 1e-6
        pnets_probs = out_dict["pnets_probs"]
        mlp_probs = out_dict["mlp_probs"]
        n_facts = pnets_probs.shape[1]
        assert pnets_probs.size(0) == mlp_probs.size(0), f"{pnets_probs.size()}-{mlp_probs.size()}"
        assert pnets_probs.size(1) == mlp_probs.size(1), f"{pnets_probs.size()}-{mlp_probs.size()}"
        for i in range(n_facts):
            # slice out the 2‑class distributions for fact i
            reference = pnets_probs[:, i, :].clamp(min=eps)
            prediction = mlp_probs[:, i, :].clamp(min=eps)
            # KL across the two‐class support
            kl_i = F.kl_div(prediction.log(), reference, reduction='batchmean')
            kl += kl_i
        kl_total += args.w_kl * (kl / n_facts)
        losses['kl'] = kl_total.item()
        #print("KL loss", kl_total.item())
    else:
        kl_total = 0.0

    return loss + args.gamma * mitigation + kl_total, losses
