...
def conv_block(in_channels, out_channels):
    """
    Returns a convolutional block: Conv2d -> BatchNorm2d -> ReLU -> MaxPool2d.
    """
...
def compute_variance(prototypes: torch.Tensor, num_classes:int) -> float:
    ...
    chi2_quantile = st.chi2.ppf(0.50, df=num_classes)
...
class PrimitivesProtoNet(nn.Module):
    ...
    def __init__(self, x_dim=3, hid_dim=64, z_dim=64, missing_classes=None):
        """
        Args:
            x_dim: Number of input channels (3 for RGB).
            hid_dim: Number of channels in the intermediate convolutional layers.
            z_dim: Number of output channels (embedding dimension).
        """
        super(PrimitivesProtoNet, self).__init__()
    ...
    def predict(self, support_images, support_labels, query_images, unknown_init=None, debug=False):
        """
        Given a support set (support_images, support_labels) and query_images,
        ...
        Args:
            ...
            support_labels: Tensor of shape [n_support_total], with labels in {0,1,2}.
        ...
        emb_dim = support_embeddings.size(1)

        # Determine which classes are present in the support set.
        # For KANDINSKY, assume full label set is 0-9.
        full_class_set = list(range(3))
        ...
        if self.num_hidden > 0 and unknown_init:
            ...
            assert known_prototypes.size(0) + self.num_hidden == 3, \
                f"Number of prototypes should be 3, but got {known_prototypes.size(0) + self.num_hidden}"
            variance = compute_variance(known_prototypes, emb_dim)
            ...
            with torch.no_grad():
                ...
                if debug: 
                    print("[DEBUG] Unknown prototypes initialized with first value: ", self.unknown_prototypes[1])
        
        if self.num_hidden > 0 and not unknown_init and debug:
            print("[DEBUG] Unknown prototype first value: ", self.unknown_prototypes[1])
        ...
        prototypes = torch.stack(prototypes)  # [3, emb_dim]
        assert prototypes.shape == (3, emb_dim), f"Concatenated prototypes shape mismatch: expected (3, {emb_dim}), got {prototypes.shape}"
        
        # Compute Euclidean distances between each query and each prototype.
        dists = euclidean_dist(query_embeddings, prototypes)
        assert dists.shape == (n_query, 3), f"Dists shape is not ({n_query},3), but {dists.shape}"
        if debug:
            print("[DEBUG] Distances: ", dists)
        
        # Apply log-softmax to the negative distances to obtain log-probabilities for each class.
        log_p_y = F.log_softmax(-dists, dim=1)
        assert log_p_y.shape == (n_query, 3), f"log_p_y shape is not ({n_query},3), but {log_p_y.shape}"
