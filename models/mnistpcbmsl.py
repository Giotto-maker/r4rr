...
class MnistPcbmSL(MnistSL):
    ...
    def __init__(
        ...
        self.mlp = torch.nn.Sequential(
            torch.nn.Linear(self.encoder.latent_dim * self.n_facts * self.n_images, 50),
            torch.nn.ReLU(),
    ...
    def forward(self, x):
        ...
        for i in range(self.n_images):
            ...
            pred_embeddings = sample_gaussian_tensors(latents, logsigma, n_samples)

            z_logits.append(pred_embeddings.permute(0, 2, 1, 3).unsqueeze(-1))
        ...
        z_logits = z_logits.view(
            -1, n_samples, self.encoder.latent_dim * self.n_facts * self.n_images
        )
        ...
        return {"CS": pCs, "YS": preds, "pCS": pCs, "MUS": mus, "LOGVARS": logvars}
