...
class BddoiaDPLDisj(SDDOIADPL):
    """DISJOINT BACKBONES DPL MODEL FOR BOIA"""
    ...
    def __init__(
        ...
        super(BddoiaDPLDisj, self).__init__(
            encoder=encoder,
        ...
        if self.args.task == "boia":
            ...
            raise NotImplementedError("Invalid task for BDDOIA")
    ...
    def forward(self, x):
        ...
        for i in range(self.n_facts):
            lc = self.encoder[i](x) # torch.Size([batch_size, 1])
            lcs.append(lc)
        cs = torch.cat(lcs, dim=1)
        assert cs.shape == (batch_size, self.n_facts),\
            f"Expected cs shape ({batch_size}, {self.n_facts}), got {cs.shape}"
