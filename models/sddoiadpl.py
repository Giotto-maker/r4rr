...
class SDDOIADPL(DeepProblogModel):
    ...
    def forward(self, x, 
        ...
        cs_modified = cs.clone()
            
        if logits_tfs is not None:
            logits_tfs = logits_tfs * (cs_max - cs_min) + cs_min 
            logits_reshaped = logits_tfs.view(-1, 1)        
            cs_modified[:, 3, :] = logits_reshaped
            #print("Logits TFS done")
        if logits_ts is not None:
            logits_ts = logits_ts * (cs_max - cs_min) + cs_min 
            logits_reshaped = logits_ts.view(-1, 1)        
            cs_modified[:, 4, :] = logits_reshaped
            #print("Logits TS done")
        if logits_cars is not None:
            logits_cars = logits_cars * (cs_max - cs_min) + cs_min 
            logits_reshaped = logits_cars.view(-1, 1)        
            cs_modified[:, 5, :] = logits_reshaped
            #print("Logits CAR done")
        if logits_peds is not None:
            logits_peds = logits_peds * (cs_max - cs_min) + cs_min 
            logits_reshaped = logits_peds.view(-1, 1)        
            cs_modified[:, 6, :] = logits_reshaped
            #print("Logits PED done")
        if logits_rid is not None:
            logits_rid = logits_rid * (cs_max - cs_min) + cs_min 
            logits_reshaped = logits_rid.view(-1, 1)        
            cs_modified[:, 7, :] = logits_reshaped
            #print("Logits RID done")
        if logits_oth is not None:
            logits_oth = logits_oth * (cs_max - cs_min) + cs_min 
            logits_reshaped = logits_oth.view(-1, 1)        
            cs_modified[:, 8, :] = logits_reshaped
            #print("Logits OTH done")

        # normalize concept preditions
        pCs = self.normalize_concepts(cs_modified)

        # Problog inference to compute worlds and query probability distributions
        py = self.problog_inference(pCs)

        cs_modified = torch.squeeze(cs_modified, dim=-1)

        return {"CS": cs_modified, "YS": py, "pCS": pCs}
