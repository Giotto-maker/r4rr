...
class KandSLSingleJoint(CExt):
    ...
    def __init__(self, encoder, n_images=3, c_split=(), args=None, n_facts=6, nr_classes=2):
        ...
        """
        super(KandSLSingleJoint, self).__init__(encoder=encoder, n_images=n_images, c_split=c_split)
    ...
    def forward(self, x, concept_extractor, transform, args):
        ...
        Args:
            x (torch.Tensor): Input tensor of shape (batch_size, 3 * n_images, 64, 64), where n_images is the number of images per sample.
            concept_extractor (callable): Function or model used to extract concepts from the input images.
            transform (callable): Transformation function to be applied to the cropped objects.
            args (Namespace or dict): Additional arguments required for the concept extraction and cropping process.
        Returns:
            dict: A dictionary containing:
                - "CS" (torch.Tensor): Latent concepts tensor of shape (batch_size, n_images, n_predicates * 2).
                - "YS" (torch.Tensor): Predicted class probabilities of shape (batch_size, nr_classes).
                - "pCS" (torch.Tensor): Normalized concepts tensor of shape (batch_size, n_images, ...).
                - "gpreds" (Any): Aggregated predicate predictions and label probabilities.
        """
        cs, pCs, preds = [], [], []
    ...
    # & Compute query method (check prokansl for details)
