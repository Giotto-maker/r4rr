...
class PROMnistDPL(DeepProblogModel):
    """DPL MODEL FOR MNIST"""
    ...
    def __init__(
        ...
        """
        super(PROMnistDPL, self).__init__(
            encoder=encoder,
    ...
    def forward(self, x, support_images, support_labels, unknown_init=False): 
        """
        Performs a forward pass through the model, processing input images and support data to produce concept logits, 
        normalized concept predictions, and probabilistic query results.
        Args:
        ...
        Returns:
            dict: A dictionary containing:
                - "CS" (torch.Tensor): Concept logits for each image, shape (batch_size, n_images, n_facts).
                - "YS" (torch.Tensor): Probabilistic predictions for queries, shape depends on problog_inference output.
                - "pCS" (torch.Tensor): Normalized concept predictions, shape (batch_size, n_images, n_facts).
        Raises:
            AssertionError: If the number of classes or input shapes do not match expected values.
        """
    ...
    def normalize_concepts(self, z, split=2):
        ...
        prob_digit1 = nn.Softmax(dim=1)(prob_digit1 / 10)
        prob_digit2 = nn.Softmax(dim=1)(prob_digit2 / 10)
