...
from backbones.addmnist_protonet import ProtoNet
import numpy as np
...
class SHORTMNIST(BaseDataset):
    ...
    def get_backbone(self):
        if self.args.joint:
            ...
            # ! new backbone for prototypical networks
            if self.args.prototypes:
                print("[BACKBONE-INFO] Using Prototypical Networks as backbone")
                # return MNISTPrototypicalNetwork(), MNISTPairsDecoder()
                return ProtoNet(missing_classes=self.args.hide), MNISTPairsDecoder()

            if self.args.model in ['mnistdplrec', 'mnistltnrec', 'mnistslrec']:
                print("[BACKBONE INFO] Using encoder-decoder backbone for reconstruction loss")
                return MNISTSingleEncoder(), MNISTPairsDecoder(latent_dim=170, c_dim=170)
