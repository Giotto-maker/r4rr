...
from backbones.kand_protonet import PrimitivesProtoNet
from backbones.kand_single_encoder_joint import SingleJointMLP
from backbones.kand_single_encoder_split import SingleDisjMLP
...
class Kandinsky(BaseDataset):
    ...
    def get_backbone(self, args=None):
        ...
        if self.args.preprocess:
            ...
            elif self.args.prototypes:  # ^ istantiate the encoder in place of None
                print("[PROTO-INFO] Using Prototypical Networks as backbone")
                return (
                    PrimitivesProtoNet(),PrimitivesProtoNet()
                    ), DecoderConv64(x_shape=(3, 64, 64), z_size=18, z_multiplier=2) 
            
            # ! new baselines
            elif self.args.model in ['kanddplsinglejoint', 
                                    'kandslsinglejoint', 
                                    'kandltnsinglejoint']:
                return (
                    SingleJointMLP(), 
            ...
            elif self.args.model in ['kanddplsingledisj',
                                      'kandslsingledisj',
                                      'kandltnsingledisj']:
                return (
                    SingleDisjMLP(), SingleDisjMLP(),
                    ), DecoderConv64(x_shape=(3, 64, 64), z_size=18, z_multiplier=2)
