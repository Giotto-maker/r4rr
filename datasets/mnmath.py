...
class MNMATH(BaseDataset):
    ...
    def get_data_loaders(self):
        ...
        return self.train_loader, self.val_loader, self.test_loader, self.ood_loader

    # ^ method to return the backbone for the MNMATH models
    def get_backbone(self):
        if self.args.backbone == "neural":
            return MNMNISTCNN(num_images=8, num_classes=2), None
        
        # ! new backbone for prototypical networks
        elif self.args.prototypes and self.args.task == "mnmath":
            print("[BACKBONE-INFO] Using Prototypical Networks as backbone")
            return ProtoNet(missing_classes=self.args.hide), None

        elif self.args.task == "mnmath":
